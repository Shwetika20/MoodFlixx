...
async function getNextMCQQuestion() {
  ...
  const prompt = `
You're a mental wellness assistant. Based on the following conversation history, generate a NEW and UNIQUE multiple-choice question that helps assess the user's mood more deeply.
...
app.post('/get-movie-description', async (req, res) => {
  ...
  const prompt = `
You are a movie critic. Given the user's mood and genre preferences, generate a movie description that matches both the emotional tone and genre:
